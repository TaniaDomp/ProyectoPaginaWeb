    ...
    if (isset($_SESSION['carrito2'])){
        if (isset($_GET['id'])){
            ...
            if ($encontro == true){
                ...
                header("location: 14-1 Sistemafis-CafetCar.php");
            }
            else{
                $nombre="";
                $precio="";
                $imagen="";
                $resultado=$conexion->query("SELECT * FROM producto_cafo WHERE id_producafo=".$_GET['id']) or die ($conexion->error);
...
<body>
    ...
    <?php 
    $total = 0;
    if ($_SESSION['carrito2']){
        $arregloCar = $_SESSION['carrito2'];
   ...
   <?php 
    ...
    <div class="colum-dere">
        <button class="btn-re" onclick="window.location = '14-1 SistemafisCompra-caf.php'">Realizar compra</button>
    </div>
    </div>
