<?php
    ...
    $conexion -> query ("INSERT INTO ventas_bolo(total_bolo, fecha_bolo, Usuario) VALUES ($total, '$fecha', '$usuario1')") or die ($conexion->error); 
    
    for ($i=0;$i<count($arreglo);$i++){
    $conexion -> query ("INSERT INTO entregas_bolo(id_prodbol, cantidad, total_bolo, fecha_bolo, Usuario) VALUES (
    ".$arreglo[$i]['Id'].",
    ".$arreglo[$i]['Cantidad'].",
    $total, 
    '$fecha', 
    ...
    $id_vbolo = $conexion -> insert_id;

    for ($i=0;$i<count($arreglo);$i++){
        $conexion -> query ("INSERT INTO productos_ventbolo(id_vbolo, id_prodbol, cantidad, precio_pbolo, subtotal_bolo) VALUES 
        ($id_vbolo, 
        ".$arreglo[$i]['Id'].",
        ".$arreglo[$i]['Cantidad'].",
        ".$arreglo[$i]['Precio'].",
...
<body>
    ...
    <center><button class="btn-re" onclick="window.location = '17 SistemaVonline-Bol.php'">Regresar a la tienda</button></center>
