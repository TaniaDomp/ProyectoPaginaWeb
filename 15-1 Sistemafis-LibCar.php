    ...
    if (isset($_SESSION['carrito5'])){
        if (isset($_GET['id'])){
            ...
            if ($encontro == true){
                ...
                $_SESSION['carrito5']=$arreglo;
                header("location: 15-1 Sistemafis-LibCar.php");
            }
            else{
                $nombre="";
                $precio="";
                $imagen="";
                $resultado=$conexion->query("SELECT * FROM producto_lib WHERE id_prodlib=".$_GET['id']) or die ($conexion->error);
...
<body>
    ...
    <div class="menu-sis">
        <div class="img-lo">
            <a href="15-1%20Sistemafis-Lib.php"><img src="Logo-supsi.jpg" alt="" class="product__img1"></a>
        </div>
        <div class="cer-se">
    ...
    <?php 
    $total = 0;
    if ($_SESSION['carrito5']){
        $arregloCar = $_SESSION['carrito5'];
   ...
   <?php 
    ...
    <div class="colum-dere">
        <button class="btn-re" onclick="window.location = '15-1 SistemafisCompra-lib.php'">Realizar compra</button>
    </div>
    </div>
