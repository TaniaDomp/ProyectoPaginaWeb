<?php
    ...
    if (!isset($_SESSION['carrito5'])){
        header('location: 15-1 Sistemafis-Lib.php');
    }
    ...
    $conexion -> query ("INSERT INTO ventasfis_lib(total_lib, fecha_lib, Usuario) VALUES ($total, '$fecha', '$usuario1')") or die ($conexion->error); 
    
    $id_vcafo = $conexion -> insert_id;

    for ($i=0;$i<count($arreglo);$i++){
        $conexion -> query ("INSERT INTO ventaspr_fislib(id_vlib, id_prodlib, cantidad, precio_plib, subtotal_lib) VALUES 
        ($id_vcafo, 
        ".$arreglo[$i]['Id'].",
        ".$arreglo[$i]['Cantidad'].",
        ".$arreglo[$i]['Precio'].",
...
<body>
   ...
   <div class= "dos-colum2"> 
    ...
    <center><button class="btn-re" onclick="window.location = '15-1 Sistemafis-Lib.php'">Regresar a la tienda</button></center>
