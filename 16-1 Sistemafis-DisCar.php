    ...
    if (isset($_SESSION['carrito8'])){
        if (isset($_GET['id'])){
            ...
            if ($encontro == true){
                ...
                $_SESSION['carrito8']=$arreglo;
                header("location: 16-1 Sistemafis-DisCar.php");
            }
            else{
                $nombre="";
                $precio="";
                $imagen="";
                $resultado=$conexion->query("SELECT * FROM producto_dis WHERE id_produdis=".$_GET['id']) or die ($conexion->error);
...
<title>Ventas del cliente</title>
</head>
<body>
    ...
    <?php 
    $total = 0;
    if ($_SESSION['carrito8']){
        $arregloCar = $_SESSION['carrito8'];
   ...
   <?php 
    ...
    <div class="colum-dere">
        <button class="btn-re" onclick="window.location = '16-1 SistemafisCompra-dis.php'">Realizar compra</button>
    </div>
    </div>
